<div>
    <div class="flex justify-between items-center mb-4">
        ...
        @if(auth()->user()->hasRole('super-admin'))
            <flux:button wire:click="create" variant="primary">Add Permission</flux:button>
        @endif
    </div>
            ...
            <tbody>
                @foreach ($permissions as $permission)
                    <tr class="border-b border-zinc-100 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition">
                        <td class="px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200 font-mono">{{ ($permissions->currentPage() - 1) * $permissions->perPage() + $loop->iteration }}</td>
                        <td class="px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200">{{ $permission->name }}</td>
                        <td class="px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200">{{ $permission->guard_name }}</td>
                        <td class="px-4 py-2 flex gap-2">
                            @if(auth()->user()->hasRole('super-admin'))
                                <flux:button wire:click="edit({{ $permission->id }})" size="sm" variant="filled">Edit</flux:button>
                                <flux:button wire:click="confirmDelete({{ $permission->id }})" size="sm" variant="danger">Delete</flux:button>
                            @endif
                        </td>
                    </tr>
                @endforeach
    ...
    <flux:modal wire:model="showModal" dismissible>
        ...
        <form wire:submit.prevent="save" class="p-4">
            <h3 class="text-lg font-semibold mb-4">{{ $editPermissionId ? 'Edit Permission' : 'Create Permission' }}</h3>
    ...
    <flux:modal wire:model="showConfirm" name="confirm" dismissible>
        ...
        <div class="p-4">
            ...
            <p class="mb-6">Are you sure you want to delete this permission? This action cannot be undone.</p>
